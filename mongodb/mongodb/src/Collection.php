...
class Collection
{
    ...
    public function count($filter = [], array $options = [])
    {
        ...
        if ( ! isset($options['readConcern']) &&  ! $this->isInTransaction($options)) {
            $options['readConcern'] = $this->readConcern;
        }
    ...
    public function mapReduce(JavascriptInterface $map, JavascriptInterface $reduce, $out, array $options = [])
    {
         ...
         * A read concern is also not compatible with transactions.
         */
        if ( ! isset($options['readConcern']) && ! ($hasOutputCollection && $this->readConcern->getLevel() === ReadConcern::MAJORITY) &&  ! $this->isInTransaction($options)) {
            $options['readConcern'] = $this->readConcern;
        }
