...
class WeecotSysRoute implements IRoute
{
    /**
     * 客户端
     * @var \stdClass
     ...
     * @var string
     */
    private $direction = 'Sercvice';//服务端
    ...
    public function handleClientRequest($request)
    {
        ...
        $this->client_data->method_name = $route[$count - 1] ?? 'antiFake';
        $this->client_data->version = $route[$count - 3] ?? 'v_1_0_1';
        unset($route[$count - 1]);
        unset($route[$count - 3]);
        unset($route[0]);
        $this->client_data->controller_name = $route[$count-2] ?? 'AntiFake';
    }
    ...
    {
//        var_dump($this->client_data);exit;
