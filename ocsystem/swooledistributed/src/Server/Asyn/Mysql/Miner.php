...
class Miner extends Child
{
    ...
    /**
     * 拼接同时修改多个字段功能(比较简陋，后期优化)
     * @return string
     */
    public function getIntoStringUpdate()
    {
        $statement = 'SET ';
        foreach ($this->intoColums as $into_key => $colum) {
            $statement .= "`$colum`=".$this->autoQuote($this->intoValues[0][$into_key]).", ";
        }
        $statement = substr($statement, 0, -2);
        return $statement;
    }
    ...
    private function getUpdateStatement($usePlaceholders = true)
    {
        ...
        if ($this->isInto) {
            $statement .= " " . $this->getIntoStringUpdate();
        } else {
            if ($this->set) {
                $statement .= " " . $this->getSetString($usePlaceholders);
            }
     ...
     * @return $this
     */
    public function info($column, $alias = null)
    {
     ...
     * @return $this
     */
    public function list($column, $page = 1, $pagesize = 20, $alias = null)
    {
        $this->selectType = 'list';
        $this->select[$column] = $alias;
        $this->limit($pagesize, $page);
        return $this;
    }
    ...
    public function limit($limit, $offset = 0)
    {
        ...
        }elseif ($this->selectType == 'list'){
            $this->limit['limit'] = $limit;
            if($offset == 0){
                $offset = 1;
            }
            $this->limit['offset'] = ($offset - 1) * $limit;
        }else{
            $this->limit['limit'] = $limit;
            $this->limit['offset'] = $offset;
        }
    ...
    public function query($sql = null, callable $set = null)
    {
        ...
        $current_page = 0;
        $total_num = 0;
        $result = $this->getProxy()->_query($sql, $set, $type);
        if(!empty($result['result']) && $this->selectType == 'info'){
        ...
        if($this->selectType == 'list'){
            //列表查询
            $current_num = count($result['result']) ? count($result['result']) : 0;
            if($this->getLimitOffset() == 0){
                $current_page = 1;
            }elseif ($this->getLimitOffset() > 0){
                $current_page = ($this->getLimitOffset() / $this->getLimit()) + 1;
            }
            $pagesize = $this->getLimit() ? $this->getLimit() : 0;
            $this->select('count(*) as total_num');
            $this->limit = null;
            $pageinfo = $this->getProxy()->_query(null, $set, 1);
            $total_num = $pageinfo['result'][0]['total_num'] ? $pageinfo['result'][0]['total_num'] : 0;
        }
        $result['pageinfo'] = [
            'pageSize'      =>  intval($pagesize),
            'currentNum'    =>  intval($current_num),
            'currentPage'   =>  intval($current_page),
            'totalNum'      =>  intval($total_num),
        ];
        $this->selectType = '';
        return $result;
    }
     ...
     * @throws \Throwable
     */
    public function _query($sql = null, callable $set = null, $type = 1)
    {
        $mySqlCoroutine = Pool::getInstance()->get(MySqlCoroutine::class);
        if (get_instance()->isTaskWorker()) {//如果是task进程自动转换为同步模式
            $this->mergeInto($this->mysql_pool->getSync());
